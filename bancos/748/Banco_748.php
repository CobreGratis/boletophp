...
class Banco_748 extends Boleto{
  ...
  * 20 – 20 01 Código numérico correspondente ao tipo de cobrança: “3” – SICREDI.
  ...
  * 37 – 41 05 Código do cedente (Numero da conta do cedente sem o digito verificador)
  * 42 – 42 01 Filler - zeros (Obs.: será 1 (um) quando houver valor expresso no campo “valor
  *            do documento”
  * 43 – 43 01 Filler – zeros
  * 44 – 44 01 DV do campo livre calculado por módulo 11 com aproveitamento total (resto igual a
 ...
 function febraban_20to44($boleto) {
   ...
   */
  $cnn = explode('-', $boleto->arguments['carteira_nosso_numero']);
  
  $lenght = array(1, 1, 2, 1);
  foreach($cnn as $key => $value){
   if(empty($cnn[$key]) || strlen($cnn[$key]) != $lenght[$key] || !is_numeric($cnn[$key])){
    $cnn[$key] = BOLETO_WARNING;
   }
  }
  
  if(in_array(BOLETO_WARNING, $cnn)) {
   $boleto->setWarning(array('carteira_nosso_numero', 'Valor invalido ou mal formado.'));
  }
  
  // Make it available for other methods to use it.
  $boleto->computed['carteira_nosso_numero'] = $cnn;
  
  // Make sure conta has 5 digits.
  $conta = $boleto->computed['conta'] = str_pad($boleto->arguments['conta'], 5, 0, STR_PAD_LEFT);
  
  // Get values to mount the base number to calculate the check digit for Nosso Numero.
  $agencia = $boleto->arguments['agencia'];
  $posto_cooperativa = $boleto->computed['carteira_nosso_numero'][2];
  $conta = $boleto->computed['conta'];
  $ano = date("y");
  $indicador_geracao = $boleto->computed['carteira_nosso_numero'][3];
  ...
  $cd = $boleto->modulo_11($cd_base);
  // Construct nosso_numero
  $boleto->computed['nosso_numero'] = "$ano/$indicador_geracao" . "$nosso_numero-" . $cd['digito'];
  
  // 20 to 21 tipo de cobrança and tipo de carteira.
  $boleto->febraban['20-44'] = $cnn[0].$cnn[1];

  // 22 to 30 Nosso Numero.
  $boleto->febraban['20-44'] .= $ano . $indicador_geracao . $nosso_numero . $cd['digito'];

  // 31 to 34 Agencia.
  $boleto->febraban['20-44'] .= $boleto->arguments['agencia']; 

  // 35 to 36 Posto da cooperativa cedente.
  $boleto->febraban['20-44'] .= $cnn[2];
  
  // 37 to 41 Código do cedente (Numero da conta do cedente sem o digito verificador)
  $boleto->febraban['20-44'] .= $conta;
  
  // 42 to 43 Filters
  $boleto->febraban['20-44'] .= 10;
  
  // 44 to 44 Check Digit of febraban 20 to 43
  $cd = $boleto->modulo_11($boleto->febraban['20-44']);
  $boleto->febraban['20-44'] .= $cd['digito'];
